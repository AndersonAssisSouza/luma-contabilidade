...
async function syncContasReceber(supabase: any, token: string, empresaId: string): Promise<number> {
  const dataInicio = new Date(); dataInicio.setMonth(dataInicio.getMonth() - 12);
  const dataFim = new Date(); dataFim.setMonth(dataFim.getMonth() + 6);
  const params = new URLSearchParams({
    data_vencimento_de: dataInicio.toISOString().split("T")[0],
    data_vencimento_ate: dataFim.toISOString().split("T")[0], page: "0", size: String(PAGE_SIZE),
  });
  let allItems: any[] = []; let page = 0; let hasMore = true;
  while (hasMore) {
    params.set("page", String(page));
    await rateLimiter.waitIfNeeded();
    const response = await fetch(`${CONTAAZUL_API_BASE}/v1/financeiro/eventos-financeiros/contas-a-receber/buscar?${params}`, { headers: { "Authorization": `Bearer ${token}` } });
    const data = await response.json();
    const items = data.itens || data.items || [];
    allItems = allItems.concat(items); hasMore = items.length >= PAGE_SIZE; page++;
  }
  const records = allItems.map((item: any) => ({
    id: item.id, empresa_id: empresaId, descricao: item.descricao || null, status: item.status,
    status_traduzido: item.status_traduzido || null, total: item.total || 0,
    pago: item.pago || 0, nao_pago: item.nao_pago || 0, data_vencimento: item.data_vencimento || null,
    data_criacao: item.data_criacao || null, data_alteracao: item.data_alteracao || null,
    pessoa_id: item.pessoa?.id || item.id_pessoa || null, categoria_id: item.categoria?.id || item.id_categoria || null,
    conta_financeira_id: item.conta_financeira?.id || null, dados_raw: item, sync_at: new Date().toISOString(),
  }));
  if (records.length > 0) await supabase.from("ca_contas_receber").upsert(records, { onConflict: "id" });
  return records.length;
}
...
async function syncContasPagar(supabase: any, token: string, empresaId: string): Promise<number> {
  ...
  while (hasMore) {
    params.set("page", String(page));
    await rateLimiter.waitIfNeeded();
    const response = await fetch(`${CONTAAZUL_API_BASE}/v1/financeiro/eventos-financeiros/contas-a-pagar/buscar?${params}`, { headers: { "Authorization": `Bearer ${token}` } });
    const data = await response.json();
    const items = data.itens || data.items || [];
    allItems = allItems.concat(items); hasMore = items.length >= PAGE_SIZE; page++;
  }
  ...
  if (records.length > 0) await supabase.from("ca_contas_pagar").upsert(records, { onConflict: "id" });
  return records.length;
}
